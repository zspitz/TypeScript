...
namespace ts.Completions.PathCompletions {
     ...
     */
    function getBaseDirectoriesFromRootDirs(rootDirs: ReadonlyArray<string>, basePath: string, scriptPath: string, ignoreCase: boolean): string[] {
    ...
    function getCompletionEntriesForDirectoryFragmentWithRootDirs(rootDirs: ReadonlyArray<string>, fragment: string, scriptPath: string, extensions: ReadonlyArray<string>, includeExtensions: boolean, span: TextSpan, compilerOptions: CompilerOptions, host: LanguageServiceHost, exclude?: string): CompletionEntry[] {
    ...
    function enumeratePotentialNonRelativeModules(fragment: string, scriptPath: string, options: CompilerOptions, typeChecker: TypeChecker, host: LanguageServiceHost): string[] {
        ...
        const ambientModules = typeChecker.getAmbientModules().map(sym => stripQuotes(unescapeLeadingUnderscores(sym.name)));
        let nonRelativeModuleNames = ambientModules.filter(moduleName => startsWith(moduleName, fragment));
    ...
    function tryReadDirectory(host: LanguageServiceHost, path: string, extensions?: ReadonlyArray<string>, exclude?: ReadonlyArray<string>, include?: string[]): string[] {
